...
export function TransaksiPage(): JSX.Element {
  ...
  return (
    <div className="animate-fade-in space-y-5 pt-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-extrabold text-white tracking-tight">Transaksi</h1>
        <p className="text-xs text-slate-500 font-semibold mt-1">Riwayat lengkap pencatatanmu</p>
      </div>

      {/* Summary Card */}
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 relative overflow-hidden">
        <div className="absolute -top-8 -right-8 w-32 h-32 bg-blue-500 rounded-full filter blur-[60px] opacity-20 pointer-events-none" />
        <div className="relative z-10">
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{summaryLabel}</span>
          <p className="text-2xl font-extrabold text-white mt-1">{formatIDR(summaryAmount)}</p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-slate-900 border border-slate-800 rounded-2xl py-3.5 pl-11 pr-4 text-xs font-semibold text-white placeholder-slate-600 focus:outline-none focus:border-slate-600 transition-colors"
        />
      </div>
          ...
          {Object.entries(grouped).map(([dateLabel, txs]) => (
            <div key={dateLabel}>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[10px] text-slate-600 font-bold">{txs.length} transaksi</span>
              </div>
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 divide-y divide-slate-800/50">
                {txs.map(t => (
